...
puts "---combien de noms ?"
puts accounts.length

puts "---combien avec un chiffre ?"
puts accounts.select {
  |login| /\d/.match(login)
}.length
puts accounts.select {
  |login| # nom de var
  login.count("0-9") > 0 #condition
}.length

puts "---combien avec aude ?"
puts accounts.select {
  |login| login.downcase.include?("aude")
}.length
...
puts "en ordre alphabétique ?"
print accounts.sort(&:casecmp)[0] +"..."
print accounts.sort(&:casecmp)[-1]
puts ""
puts "les 50 les plus courts ?"
sorted = accounts.sort_by {
  |name|
  name.size
}
puts sorted[0] + "..." + sorted[49]

puts "ou est @epenser ?"
puts accounts.find_index("@epenser")
